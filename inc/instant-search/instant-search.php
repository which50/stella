...
if ( ! function_exists( 'vw_instant_search' ) ) {
	function vw_instant_search() {
		...
		$vw_secondary_query = new WP_Query( apply_filters( 'vw_filter_instant_search_query_args', $query_args ) );
		...
		$html = '';
		while ( $vw_secondary_query->have_posts() ) { $vw_secondary_query->the_post();
			// global $id;
			// var_dump($id,$post);
		...
		$html .= '<li class="vw-instant-search-all-result">';		$html .= '<a href="http://which-50.com/?s='.$q.'"><span class="vw-instant-search-result-title vw-header-font">'.__( 'View all results &raquo;', 'envirra' ).'</span></a>';
...
if ( ! function_exists( 'vw_instant_search_add_icon_to_mobile_nav' ) ) {
	function vw_instant_search_add_icon_to_mobile_nav( $content ) {
		...
		<span class="vw-search-buton main-menu-item">
			<a class="vw-search-buton" href="<?php echo esc_attr( get_search_link() ); ?>"><i class="vw-icon icon-iconic-search"></i></a>
		</span>
		<?php
