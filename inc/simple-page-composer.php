			...
			'post_slider' => array(
				...
				'options' => array(
					...
					'additional_posts' => array(
						'title' => 'Show additional posts',
						'description' => 'Show 4 featured posts under slider',
						'field' => 'select',
						'default' => 'show',
						'options' => array(
							'show' => 'Show',
...
if ( ! function_exists( 'vw_render_spc_section_post_slider' ) ) {
	function vw_render_spc_section_post_slider( $args ) {
		...
		$additional_posts = get_post_meta( $page_id, $field_prefix.'_additional_posts', true );
		...
		if ( 'show' == $additional_posts ) {
			...
			echo '<div class="container vw-additional-posts test4"><div class="row"><div class="col-md-12">';
			
			$vw_secondary_query = new WP_Query( $additional_query );

			get_template_part( 'templates/post-loop/loop-additional-posts' );
...
if ( ! function_exists( 'vw_render_spc_section_post_box' ) ) {
	function vw_render_spc_section_post_box( $args ) {
		...
		$vw_secondary_query = new WP_Query( apply_filters( 'vw_filter_spc_post_box_query', $query_args ) );
...
//		);
		...
		// ==== End temp query =====================================
		vw_enable_avoid_duplicate_post();
		$vw_secondary_query = new WP_Query( apply_filters( 'vw_filter_spc_post_box_sidebar_query', $query_args ) );
...
if ( ! function_exists( 'vw_render_spc_section_custom_content' ) ) {
	function vw_render_spc_section_custom_content( $args ) {
		...
		$title = get_post_meta( $page_id, $field_prefix.'_title', true );
		$subtitle = get_post_meta( $page_id, $field_prefix.'_subtitle', true );
		$content = get_post_meta( $page_id, $field_prefix.'_content', true );
		$sidebar = get_post_meta( $page_id, $field_prefix.'_sidebar', true );
		...
		<?php
		echo apply_filters( 'the_content', apply_filters( 'vw_filter_spc_custom_content_body', $content ) );
		echo '</div>';
