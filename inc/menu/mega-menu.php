...
if ( ! function_exists( 'vw_mega_menu_render_as_category_recent' ) ) {
	function vw_mega_menu_render_as_category_recent( $item, $depth ) {
		...
		if ( $item->vw_menu_type == 'category' ) { 
			...
			$vw_secondary_query = new WP_Query( apply_filters( 'vw_filter_mega_menu_category_query_args', $query_args ) );
...
/**
 * Show latest products on mega menu
 */
add_action( 'vw_action_mega_menu_render_as_latest-products', 'vw_mega_menu_render_as_product_recent', 10, 2 );
if ( ! function_exists( 'vw_mega_menu_render_as_product_recent' ) ) {
	function vw_mega_menu_render_as_product_recent( $item, $depth ) {
		global $vw_secondary_query;

		if ( $item->vw_menu_type == 'latest-products' ) { 
			$query_args = array(
				'post_type' => 'product',
				'posts_per_page' => 3,
				'offset'=> 0,
			...
			$vw_secondary_query = new WP_Query( apply_filters( 'vw_filter_mega_menu_latest-products_query_args', $query_args ) );
