		...
		<p class="description description-wide vw-mega-menu-option">
			<label for="edit-menu-item-vw-menu-type-<?php echo $item_id; ?>">
				...
				<select class="widefat edit-menu-item-vw-menu-type" id="edit-menu-item-vw-menu-type-<?php echo $item_id; ?>" name="menu-item-vw-menu-type[<?php echo $item_id; ?>]">
					<option value="classic" <?php selected( $item->vw_menu_type, 'classic' ); ?>><?php echo 'Classic Menu'; ?></option>
					<option value="category" <?php selected( $item->vw_menu_type, 'category' ); ?>><?php echo 'Posts in Category (Mega Menu)'; ?></option>
					<option value="links-4-cols" <?php selected( $item->vw_menu_type, 'links-4-cols' ); ?>><?php echo '4-Columns Links (Mega Menu)'; ?></option>

					<option value="latest-products" <?php selected( $item->vw_menu_type, 'latest-products' ); ?>><?php echo 'Latest Products (Mega Menu)'; ?></option>
				</select>
			</label>
		</p>
		<?php
