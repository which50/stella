 ...
 * Content Slider
 * -------------------------------------------------------------------------- */
if ( ! function_exists( 'vw_shortcode_content_slider' ) ) {
	function vw_shortcode_content_slider( $atts, $content = null ) {
		global $vw_shortcode_content_slider;
		$vw_shortcode_content_slider = array();

		// Parse inner shortcode
		do_shortcode( $content );

		$html = '<div class="vw-content-slider">';
		$html .= '<div class="swiper-container vw-swiper-content-slider-arrow-nav">';

		$html .='<div class="swiper-pagination swiper-pagination-top"></div>';

		$html .= '<div class="swiper-wrapper">';
		$html .= implode( $vw_shortcode_content_slider, '' );
		$html .= '</div>';

		$html .='<div class="swiper-pagination swiper-pagination-bottom"></div>';

		// $html .='<div class="swiper-pagination swiper-pagination-bottom"></div><div class="swiper-button-next swiper-button-next-bottom"></div><div class="swiper-button-prev swiper-button-prev-bottom"></div>';

		$html .= '</div>';
		$html .= '</div>';

		unset( $vw_shortcode_content_slider );
		return $html;
	}
}
 ...
 * Content Slide
 * -------------------------------------------------------------------------- */
if ( ! function_exists( 'vw_shortcode_content_slide' ) ) {
	function vw_shortcode_content_slide( $atts, $content = null ) {
		$html = '<div class="swiper-slide">' . do_shortcode( $content ) . "</div>";

		global $vw_shortcode_content_slider;
		$vw_shortcode_content_slider[] = $html;
		return $html;
	}
}
...
if ( ! function_exists( 'vw_shortcode_title' ) ) {
	function vw_shortcode_title( $atts, $content = null ) {
		...
		return "<h3 class='vw-title-shortcode'><span>" . $content . "</span></h3>";
	}
}
...
if ( ! function_exists( 'vwsce_field_render_icon' ) ) {
	function vwsce_field_render_icon() {
		?>
		<link rel="stylesheet" type="text/css" href="<?php echo esc_url( get_template_directory_uri() ); ?>/components/font-icons/iconic/css/iconic.css">
		<?php
	}
