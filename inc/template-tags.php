...
if ( ! function_exists( 'vw_the_pagination' ) ) {
	function vw_the_pagination() {
		...
		if ( empty( $vw_secondary_query ) || $vw_secondary_query->max_num_pages < 2 ) {
			return;
		}
		...
		// Support offset
		if ( ! empty( $vw_secondary_query->query['offset'] ) ) {
			$total_page = ceil( ( $vw_secondary_query->found_posts - $vw_secondary_query->query['offset'] ) / $vw_secondary_query->query['posts_per_page'] );
		} else {
			$total_page = max( 1, $vw_secondary_query->max_num_pages );
		}
...
if ( ! function_exists( 'vw_the_site_social_profiles' ) ) {
	function vw_the_site_social_profiles() {
		...
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-delicious" href="%s" title="Delicious"><i class="vw-icon icon-social-delicious"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_digg' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-digg" href="%s" title="Digg"><i class="vw-icon icon-social-digg"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_dribbble' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-dribbble" href="%s" title="Dribbble"><i class="vw-icon icon-social-dribbble"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_facebook' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-facebook" href="%s" title="Facebook"><i class="vw-icon icon-social-facebook"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_flickr' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-flickr" href="%s" title="Flickr"><i class="vw-icon icon-social-flickr"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_forrst' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-forrst" href="%s" title="Forrst"><i class="vw-icon icon-social-forrst"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_github' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-github" href="%s" title="GitHub"><i class="vw-icon icon-social-github"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_googleplus' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-gplus" href="%s" title="Google+"><i class="vw-icon icon-social-gplus"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_instagram' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-instagram" href="%s" title="Instagram"><i class="vw-icon icon-social-instagram"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_linkedin' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-linkedin" href="%s" title="LinkedIn"><i class="vw-icon icon-social-linkedin"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_lastfm' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-lastfm" href="%s" title="Last.fm"><i class="vw-icon icon-social-lastfm"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_pinterest' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-pinterest" href="%s" title="Pinterest"><i class="vw-icon icon-social-pinterest"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_rss' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-rss" href="%s" title="RSS"><i class="vw-icon icon-social-rss"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_skype' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-skype" href="%s" title="Skype"><i class="vw-icon icon-social-skype"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_tumblr' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-tumblr" href="%s" title="Tumblr"><i class="vw-icon icon-social-tumblr"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_twitter' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-twitter" href="%s" title="Twitter"><i class="vw-icon icon-social-twitter"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_vimeo' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-vimeo" href="%s" title="Vimeo"><i class="vw-icon icon-social-vimeo"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_yahoo' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-yahoo" href="%s" title="Yahoo"><i class="vw-icon icon-social-yahoo"></i></a>', esc_url( $url ) );

		$url = vw_get_theme_option( 'social_youtube' );
		if ( ! empty( $url ) ) printf( '<a class="vw-site-social-profile-icon vw-site-social-youtube" href="%s" title="Youtube"><i class="vw-icon icon-social-youtube"></i></a>', esc_url( $url ) );
	...
	function vw_the_category( $classes='' ) {
		...
		if( $categories ){
			...
			foreach( $categories as $category ) {
				...
				echo '<a class="'.esc_attr( $classes ).'" href="'.get_category_link( $category->term_id ).'" title="' . esc_attr( sprintf( __( "View all posts in %s", 'envirra' ), $category->name ) ) . '" rel="category">'.$category->name.'</a>';
			}
			echo '</div></div>';
		}
	}
								...
								<a class="" href="<?php echo esc_url( $full_image_url[0] ); ?>" title="<?php the_title_attribute(); ?>" rel="bookmark" <?php vw_itemprop( 'image' ); vw_itemtype( 'ImageObject' ); ?>>
									...
									<?php 
										...
										if ( ! empty( $image_caption ) ) {
											printf( '<div class="vw-embeded-image-caption"><i class="vw-icon icon-iconic-camera"></i> %s</div>', esc_html( $image_caption ) );
										}
									?>
								</a>
							</div>
	...
	function vw_the_post_footer_sections() {
		...
		foreach ( $sections['enabled'] as $slug => $label ) {
			if ( 'post-navigation' == $slug ) {
				...
				global $vw_secondary_query;
				$vw_secondary_query = vw_get_related_posts( vw_get_theme_option( 'related_post_count' ) );
				
				if ( $vw_secondary_query->have_posts() ) {
...
if ( ! function_exists( 'vw_the_author' ) ) {
	function vw_the_author( $avatar_size = VW_CONST_AVATAR_SIZE_SMALL ) {
		?>
		<span class="vw-post-author" <?php vw_itemprop('author'); vw_itemtype('Person'); ?>>
			<?php vw_the_author_avatar( null, $avatar_size ); ?>
...
if ( ! function_exists( 'vw_the_author_avatar' ) ) {
	function vw_the_author_avatar( $author = null, $size = VW_CONST_AVATAR_SIZE_MEDIUM ) {
		if ( ! $author ) {
...
if ( ! function_exists( 'vw_the_post_date' ) ) {
	function vw_the_post_date() {
		?><i class="vw-icon icon-entypo-clock"></i> <a href="<?php the_permalink(); ?>" class="vw-post-date updated" title="<?php printf( esc_attr__('Permalink to %s', 'envirra'), the_title_attribute('echo=0') ); ?>" rel="bookmark"><?php /*?><time <?php vw_itemprop('datePublished'); ?> datetime="<?php echo get_the_time('c'); ?>"><?php echo get_the_time( get_option('date_format') ); ?></time><?php */?><time><?php echo get_the_time( get_option('date_format') ); ?></time></a><?php
	}
}
...
if ( ! function_exists( 'vw_build_post_slider_query' ) ) {
	function vw_build_post_slider_query( $args=array() ) {
		...
		return apply_filters( 'vw_filter_the_post_slider_query', $query_args );
	}
}
...
if ( ! function_exists( 'vw_the_post_slider' ) ) {
	function vw_the_post_slider( $args=array() ) {
		global $vw_secondary_query;
		$vw_secondary_query = new WP_Query( vw_build_post_slider_query( $args ) );
